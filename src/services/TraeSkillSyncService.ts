...
export class TraeSkillSyncService {
  ...
  private skillToMarkdown(skill: SkillRow): string {
    ...
    if (instructions) {
      ...
      lines.push(
        `This skill provides: ${skill.description || 'No detailed instructions available.'}`
      );
      lines.push('');
  ...
  private async cleanupDeletedSkills(activeSkills: SkillRow[]): Promise<void> {
    ...
    const activeFilenames = new Set(activeSkills.map(s => this.sanitizeFilename(s.name) + '.md'));
